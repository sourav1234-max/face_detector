...
function syncStaticGalleryFile(photos = ramGalleryCache) {
  if (!Array.isArray(photos)) return;
  try {
    const publicPhotos = photos.filter(photo => {
    ...
    fs.writeFile(galleryDbPath, JSON.stringify(catalogSnippet, null, 2), err => {
      if (err && !isReadOnlyFsError(err)) {
        console.warn('[Static Gallery Sync] Warning saving public/gallery.json:', err.message);
      }
    });
  } catch (e) {}
}
...
// --- System & Backend Monitoring State ---
...
let haStatusState = {
  ...
  lastSwitchReason: 'Single backend architecture (Railway)',
...
async function loadHAStatusFromFirestore() {
  ...
  try {
    ...
    if (snap.exists) {
      ...
      haStatusState = { ...haStatusState, ...data, activeBackend: 'railway', autoFailover: false };
    } else {
...
async function updateBackendCacheMetricsInFirestore() {
  ...
  try {
    ...
    haStatusState.railwayCache = cacheData;
    await db.doc(HA_STATUS_DOC).set({ railwayCache: cacheData, activeBackend: 'railway' }, { merge: true });
    metrics.firestoreWrites += 1;
  } catch (err) {
...
async function setActiveBackend(targetBackend = 'railway', reason = 'Single backend architecture') {
  haStatusState.activeBackend = 'railway';
  haStatusState.lastSwitchTime = new Date().toISOString();
  haStatusState.lastSwitchReason = reason;
  return { ...haStatusState, currentPlatform: CURRENT_PLATFORM, isCurrentActive: true };
}

async function toggleAutoFailover(enabled = false) {
  haStatusState.autoFailover = false;
  return { ...haStatusState, currentPlatform: CURRENT_PLATFORM, isCurrentActive: true };
}
...
async function initRamCache() {
  ...
  ramInitPromise = (async () => {
    ...
    isRamCacheInitialized = true;
    syncStaticGalleryFile(ramGalleryCache);
    return true;
  })();
