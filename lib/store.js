...
function applyEnvOverrides(settings) {
  ...
  if (process.env.GOOGLE_CLIENT_ID) merged.googleClientId = process.env.GOOGLE_CLIENT_ID;
  if (process.env.GOOGLE_CLIENT_SECRET) merged.googleClientSecret = process.env.GOOGLE_CLIENT_SECRET;
  if (process.env.GOOGLE_REFRESH_TOKEN) merged.googleRefreshToken = process.env.GOOGLE_REFRESH_TOKEN;
...
async function writeSettings(data) {
  ...
  if (isFirebaseEnabled()) {
    try {
      const db = getDb();
      // Do not persist env-only secrets if they came only from env — still OK to store admin UI settings
  ...
  return writeLocalSettings(data);
}
