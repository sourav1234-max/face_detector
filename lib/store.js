...
function getCacheStatus() {
  return {
    ...
    totalDescriptors: cacheProgress.loadedDescriptors || 0,
...
async function updateBackendCacheMetricsInFirestore() {
  ...
  try {
    ...
    const cacheData = {
      ...
      descriptors: cacheProgress.loadedDescriptors || 0,
  ...
  try {
    ...
    db.doc(HA_STATUS_DOC).onSnapshot(snap => {
      ...
      if (prevActive !== newActive) {
        ...
        // Only rebuild cache if becoming active AND cache is not yet initialized
        if ((currentPlatformKey === newActive || (newActive === 'railway' && currentPlatformKey === 'local')) && !isRamCacheInitialized) {
          console.log(`[HA Backend Activation] ${CURRENT_PLATFORM} is now ACTIVE! Rebuilding RAM cache from Firestore...`);
