...
function sanitizeSettingsForClient(settings) {
  return {
    ...
    faceDetectionEnabled: settings.faceDetectionEnabled !== false, // default true
...
function readLocalGallery() {
  ...
  return [...galleryCache].sort((a, b) => String(b.timestamp || b.uploadTime || '').localeCompare(String(a.timestamp || a.uploadTime || '')));
}
...
async function writeLocalEvents(data) {
  eventsCache = data;
  try {
    await fs.promises.writeFile(eventsDbPath, JSON.stringify(eventsCache, null, 2));
    return true;
  } catch (err) {
    if (isReadOnlyFsError(err)) {
...
async function writeEventsDb(data) {
  if (isFirebaseEnabled()) {
    const db = getDb();
    const existing = await db.collection(EVENTS_COLLECTION).get();
    const batch = db.batch();
    existing.docs.forEach(doc => batch.delete(doc.ref));
    data.forEach(evt => {
      const { id, ...rest } = evt;
      batch.set(db.collection(EVENTS_COLLECTION).doc(id), rest);
    });
    await batch.commit();
    return true;
  }
  return writeLocalEvents(data);
}
...
async function getEventById(id) {
  ...
  return readLocalEvents().find(e => e.id === id) || null;
}
