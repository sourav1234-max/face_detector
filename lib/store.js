const fs = require('fs');
const path = require('path');
const { isFirebaseEnabled, getDb, getBucket } = require('./firebase');
const { addSystemLog, CURRENT_PLATFORM } = require('./system-monitor');

const PHOTOS_COLLECTION = 'photos';
const EVENTS_COLLECTION = 'events';
const SETTINGS_DOC = 'config/settings';

const galleryDbPath = path.join(__dirname, '..', 'public', 'gallery.json');
const eventsDbPath = path.join(__dirname, '..', 'public', 'events.json');
const settingsPath = path.join(__dirname, '..', 'settings.json');

// --- In-Memory RAM Cache ---
let ramGalleryCache = null;
let ramEventsCache = null;
let ramSettingsCache = null;
let isRamCacheInitialized = false;
let ramInitPromise = null;

// Progress Tracking for RAM Cache Startup
const cacheProgress = {
  status: 'idle', // 'idle' | 'loading' | 'ready' | 'error'
  percent: 0,
  stage: 'Not started',
  loadedPhotos: 0,
  loadedEvents: 0,
  loadedDescriptors: 0,
  cacheSizeMB: 0,
  initTimeMs: 0,
  lastRefresh: null,
  error: null
};

// Request Deduplication Buffer
const processedRequestIds = new Set();
const MAX_PROCESSED_IDS = 10000;

function isDuplicateRequest(reqId) {
  if (!reqId) return false;
  if (processedRequestIds.has(reqId)) return true;
  processedRequestIds.add(reqId);
  if (processedRequestIds.size > MAX_PROCESSED_IDS) {
    const oldestId = processedRequestIds.values().next().value;
    processedRequestIds.delete(oldestId);
  }
  return false;
}

let isWritingGallery = false;
const galleryWriteQueue = [];
let settingsMemoryCache = null;
let settingsStorageReadOnly = false;
let galleryMemoryCache = null;
let eventsMemoryCache = null;
let galleryStorageReadOnly = false;

function isReadOnlyFsError(err) {
  return err && ['EACCES', 'EPERM', 'EROFS'].includes(err.code);
}

const DEFAULT_SETTINGS = {
  adminPassword: 'admin123',
  publicGalleryEnabled: true,
  publicGalleryHeading: 'Gallery Catalog',
  defaultPublicEventId: 'all',
  allowPublicFaceAdjustment: true,
  faceDetectionEnabled: true, // Global face detection pipeline toggle
  logoWidth: 245,
  photoRetentionHours: 0, // 0 = never auto-delete
  googleClientId: '',
  googleClientSecret: '',
  googleRefreshToken: '',
  googleConnectedEmail: '',
  googleHasDriveScope: false,
  galleryMessage: '',
  studioName: 'স্মৃতি চিত্র (Smriti Chitra)',
  studioOwner: 'Subhajit Pratihar',
  studioPhone: '8388086844',
  studioLocation: 'Majuria, Bankura',
  studioMapUrl: 'https://maps.google.com/?q=Majuria,Bankura'
};

function applyEnvOverrides(settings) {
  const merged = { ...settings };
  if (process.env.ADMIN_PASSWORD) merged.adminPassword = process.env.ADMIN_PASSWORD;
  if (process.env.GOOGLE_CLIENT_ID && !merged.googleClientId) {
    merged.googleClientId = process.env.GOOGLE_CLIENT_ID;
  }
  if (process.env.GOOGLE_CLIENT_SECRET && !merged.googleClientSecret) {
    merged.googleClientSecret = process.env.GOOGLE_CLIENT_SECRET;
  }
  if (process.env.GOOGLE_REFRESH_TOKEN && !merged.googleRefreshToken) {
    merged.googleRefreshToken = process.env.GOOGLE_REFRESH_TOKEN;
  }
  if (process.env.PHOTO_RETENTION_HOURS !== undefined && process.env.PHOTO_RETENTION_HOURS !== '') {
    merged.photoRetentionHours = parseFloat(process.env.PHOTO_RETENTION_HOURS);
  }
  return merged;
}

function invalidateSettingsCache() {
  // RAM settings cache is updated automatically in-memory
}

function sanitizeSettingsForClient(settings) {
  return {
    publicGalleryEnabled: settings.publicGalleryEnabled,
    publicGalleryHeading: settings.publicGalleryHeading || 'Gallery Catalog',
    defaultPublicEventId: settings.defaultPublicEventId || 'all',
    allowPublicFaceAdjustment: settings.allowPublicFaceAdjustment !== false,
    faceDetectionEnabled: settings.faceDetectionEnabled !== false,
    logoWidth: settings.logoWidth,
    photoRetentionHours: settings.photoRetentionHours,
    googleClientId: settings.googleClientId || '',
    googleClientSecret: settings.googleClientSecret ? '********' : '',
    googleDriveConnected: !!settings.googleRefreshToken,
    googleConnectedEmail: settings.googleConnectedEmail || '',
    googleHasDriveScope: settings.googleHasDriveScope !== false,
    galleryMessage: settings.galleryMessage || '',
    studioName: settings.studioName || 'স্মৃতি চিত্র (Smriti Chitra)',
    studioOwner: settings.studioOwner || 'Subhajit Pratihar',
    studioPhone: settings.studioPhone || '8388086844',
    studioLocation: settings.studioLocation || 'Majuria, Bankura',
    studioMapUrl: settings.studioMapUrl || 'https://maps.google.com/?q=Majuria,Bankura'
  };
}

// --- Operational Metrics & Google Drive Metrics Tracker ---
const metrics = {
  firestoreReads: 0,
  firestoreWrites: 0,
  firestoreDeletes: 0,
  ramCacheHits: 0,
  startTime: new Date().toISOString()
};

const driveMetrics = {
  totalUploaded: 0,
  totalFailures: 0,
  lastUploadTime: null,
  status: 'Disconnected'
};

function countDescriptors(photos) {
  if (!Array.isArray(photos)) return 0;
  let count = 0;
  photos.forEach(p => {
    if (p.descriptors && Array.isArray(p.descriptors)) {
      count += p.descriptors.length;
    }
  });
  return count;
}

function calculateCacheSizeMB() {
  try {
    const raw = JSON.stringify(ramGalleryCache || []) + JSON.stringify(ramEventsCache || []) + JSON.stringify(ramSettingsCache || {});
    return Math.round((Buffer.byteLength(raw, 'utf8') / 1024 / 1024) * 100) / 100;
  } catch (e) {
    return 0;
  }
}

function getCacheStatus() {
  return {
    isInitialized: isRamCacheInitialized,
    progress: { ...cacheProgress },
    totalPhotos: ramGalleryCache ? ramGalleryCache.length : 0,
    totalEvents: ramEventsCache ? ramEventsCache.length : 0,
    totalDescriptors: countDescriptors(ramGalleryCache),
    cacheSizeMB: cacheProgress.cacheSizeMB
  };
}

function getMetrics() {
  const totalOps = metrics.firestoreReads + metrics.ramCacheHits;
  const efficiency = totalOps > 0 ? ((metrics.ramCacheHits / totalOps) * 100).toFixed(1) + '%' : '100%';
  return {
    ...metrics,
    efficiency,
    totalPhotos: ramGalleryCache ? ramGalleryCache.length : 0,
    totalEvents: ramEventsCache ? ramEventsCache.length : 0,
    driveMetrics: { ...driveMetrics }
  };
}

// --- Active-Passive High Availability (HA) & Dual Backend State ---
const HA_STATUS_DOC = 'config/ha_status';

let haStatusState = {
  activeBackend: 'railway', // 'railway' | 'render'
  autoFailover: true,
  lastSwitchTime: new Date().toISOString(),
  lastSwitchReason: 'System initialization',
  railwayCache: { ready: false, photos: 0, descriptors: 0, events: 0, sizeMB: 0, lastSync: null },
  renderCache: { ready: false, photos: 0, descriptors: 0, events: 0, sizeMB: 0, lastSync: null }
};
let haListenerAttached = false;

async function loadHAStatusFromFirestore() {
  if (!isFirebaseEnabled()) return haStatusState;
  try {
    const db = getDb();
    if (!db) return haStatusState;

    const snap = await db.doc(HA_STATUS_DOC).get();
    metrics.firestoreReads += 1;
    if (snap.exists) {
      const data = snap.data();
      haStatusState = { ...haStatusState, ...data };
    } else {
      await db.doc(HA_STATUS_DOC).set(haStatusState);
      metrics.firestoreWrites += 1;
    }
  } catch (err) {
    console.warn('[HA Status] Failed to read/write HA status in Firestore:', err.message);
  }
  return haStatusState;
}

async function updateBackendCacheMetricsInFirestore() {
  if (!isFirebaseEnabled()) return;
  try {
    const db = getDb();
    if (!db) return;
    const isRailway = CURRENT_PLATFORM.toUpperCase() === 'RAILWAY' || CURRENT_PLATFORM.toUpperCase() === 'LOCAL';
    const fieldKey = isRailway ? 'railwayCache' : 'renderCache';
    const cacheData = {
      ready: isRamCacheInitialized,
      photos: ramGalleryCache ? ramGalleryCache.length : 0,
      descriptors: countDescriptors(ramGalleryCache),
      events: ramEventsCache ? ramEventsCache.length : 0,
      sizeMB: cacheProgress.cacheSizeMB || 0,
      lastSync: new Date().toISOString()
    };
    haStatusState[fieldKey] = cacheData;
    await db.doc(HA_STATUS_DOC).set({ [fieldKey]: cacheData }, { merge: true });
    metrics.firestoreWrites += 1;
  } catch (err) {
    console.warn('[HA Status] Failed to update cache metrics in Firestore:', err.message);
  }
}

function setupHASnapshotListener() {
  if (haListenerAttached || !isFirebaseEnabled()) return;
  try {
    const db = getDb();
    if (!db) return;
    haListenerAttached = true;
    db.doc(HA_STATUS_DOC).onSnapshot(snap => {
      if (!snap || !snap.exists) return;
      const data = snap.data();
      const prevActive = haStatusState.activeBackend;
      haStatusState = { ...haStatusState, ...data };

      const currentPlatformKey = CURRENT_PLATFORM.toLowerCase();
      const newActive = String(data.activeBackend || 'railway').toLowerCase();

      if (prevActive !== newActive) {
        console.log(`[HA Snapshot] Active backend switched: ${prevActive} -> ${newActive}`);
        addSystemLog({
          backend: CURRENT_PLATFORM,
          eventType: 'FAILOVER',
          status: 'INFO',
          details: `Active backend mode changed from ${prevActive.toUpperCase()} to ${newActive.toUpperCase()}`
        });

        // If this backend is now the active backend, clear RAM cache and rebuild from Firestore
        if (currentPlatformKey === newActive || (newActive === 'railway' && currentPlatformKey === 'local')) {
          console.log(`[HA Backend Activation] ${CURRENT_PLATFORM} is now ACTIVE! Clearing RAM cache and rebuilding from Firestore...`);
          refreshRamCache().catch(err => {
            console.error(`[HA Backend Activation Error] Cache rebuild failed:`, err.message);
          });
        }
      }
    }, err => {
      console.warn('[HA Snapshot Error]:', err.message);
    });
  } catch (err) {
    console.warn('[HA Listener Setup Error]:', err.message);
  }
}

let photosListenerAttached = false;

function setupPhotosSnapshotListener() {
  if (photosListenerAttached || !isFirebaseEnabled()) return;
  try {
    const db = getDb();
    if (!db) return;
    photosListenerAttached = true;
    db.collection(PHOTOS_COLLECTION).onSnapshot(snapshot => {
      if (!snapshot || !isRamCacheInitialized || !ramGalleryCache) return;
      snapshot.docChanges().forEach(change => {
        const data = { id: change.doc.id, ...change.doc.data() };
        const idx = ramGalleryCache.findIndex(p => p.id === data.id);
        if (change.type === 'added') {
          if (idx === -1) {
            ramGalleryCache.unshift(data);
          } else {
            Object.assign(ramGalleryCache[idx], data);
          }
        } else if (change.type === 'modified') {
          if (idx !== -1) {
            Object.assign(ramGalleryCache[idx], data);
          } else {
            ramGalleryCache.unshift(data);
          }
        } else if (change.type === 'removed') {
          if (idx !== -1) {
            ramGalleryCache.splice(idx, 1);
          }
        }
      });
    }, err => {
      console.warn('[Photos Snapshot Error]:', err.message);
    });
  } catch (err) {
    console.warn('[Photos Listener Setup Error]:', err.message);
  }
}

async function setActiveBackend(targetBackend, reason = 'Administrator request') {
  const backendKey = String(targetBackend).toLowerCase();
  if (backendKey !== 'railway' && backendKey !== 'render') {
    throw new Error("Invalid backend target. Must be 'railway' or 'render'");
  }

  haStatusState.activeBackend = backendKey;
  haStatusState.lastSwitchTime = new Date().toISOString();
  haStatusState.lastSwitchReason = reason;

  if (isFirebaseEnabled()) {
    const db = getDb();
    if (db) {
      await db.doc(HA_STATUS_DOC).set({
        activeBackend: backendKey,
        lastSwitchTime: haStatusState.lastSwitchTime,
        lastSwitchReason: reason
      }, { merge: true });
      metrics.firestoreWrites += 1;
    }
  }

  addSystemLog({
    backend: CURRENT_PLATFORM,
    eventType: 'FAILOVER',
    status: 'SUCCESS',
    details: `Active backend set to ${backendKey.toUpperCase()} | Reason: ${reason}`
  });

  return { ...haStatusState };
}

async function toggleAutoFailover(enabled) {
  haStatusState.autoFailover = !!enabled;
  if (isFirebaseEnabled()) {
    const db = getDb();
    if (db) {
      await db.doc(HA_STATUS_DOC).set({ autoFailover: haStatusState.autoFailover }, { merge: true });
      metrics.firestoreWrites += 1;
    }
  }
  return { ...haStatusState };
}

function getHAStatus() {
  const currentKey = CURRENT_PLATFORM.toLowerCase();
  const activeKey = String(haStatusState.activeBackend || 'railway').toLowerCase();
  const isCurrentActive = currentKey === activeKey || (activeKey === 'railway' && currentKey === 'local');
  return {
    ...haStatusState,
    currentPlatform: CURRENT_PLATFORM,
    isCurrentActive
  };
}

// ---------- RAM Cache Initialization ----------

async function initRamCache() {
  if (isRamCacheInitialized) {
    return true;
  }
  if (ramInitPromise) {
    return ramInitPromise;
  }

  const startTime = Date.now();
  cacheProgress.status = 'loading';
  cacheProgress.percent = 10;
  cacheProgress.stage = 'Connecting to database...';

  console.log(`[RAM Cache Startup] Server Starting... Platform: ${CURRENT_PLATFORM}`);
  addSystemLog({
    backend: CURRENT_PLATFORM,
    eventType: 'CACHE',
    status: 'INFO',
    details: `[RAM Cache Startup] Platform: ${CURRENT_PLATFORM} | Initializing metadata cache from Firestore...`
  });

  ramInitPromise = (async () => {
    if (isFirebaseEnabled()) {
      const maxRetries = 3;
      let attempt = 0;
      let success = false;
      let lastErr = null;

      // 1. Sync HA Status doc & setup realtime listeners
      await loadHAStatusFromFirestore();
      setupHASnapshotListener();
      setupPhotosSnapshotListener();

      while (attempt < maxRetries && !success) {
        attempt++;
        try {
          console.log(`[RAM Cache Startup] Initializing Firebase (Attempt ${attempt}/${maxRetries})...`);
          const db = getDb();
          if (!db) {
            throw new Error('Firestore instance not available (Authentication or Initialization failed)');
          }

          console.log('[RAM Cache Startup] Firebase Connected. Loading photo metadata & face descriptors...');
          cacheProgress.percent = 30;
          cacheProgress.stage = 'Loading photo metadata & face descriptors...';

          // 1. Single load of all photo metadata & face descriptors from Firestore
          const photosSnap = await db.collection(PHOTOS_COLLECTION).get();
          metrics.firestoreReads += photosSnap.docs.length;
          const photos = photosSnap.docs.map(doc => ({ id: doc.id, ...doc.data() }));
          photos.sort((a, b) => String(b.timestamp || b.uploadTime || '').localeCompare(String(a.timestamp || a.uploadTime || '')));
          ramGalleryCache = photos;

          const descCount = countDescriptors(photos);
          cacheProgress.loadedPhotos = photos.length;
          cacheProgress.loadedDescriptors = descCount;
          console.log(`[RAM Cache Startup] Loaded ${photos.length} photos and ${descCount} face descriptors.`);

          // 2. Single load of all events from Firestore
          cacheProgress.percent = 70;
          cacheProgress.stage = 'Loading event catalogs...';
          console.log('[RAM Cache Startup] Loading events catalog...');

          const eventsSnap = await db.collection(EVENTS_COLLECTION).get();
          metrics.firestoreReads += eventsSnap.docs.length;
          const events = eventsSnap.docs.map(doc => ({ id: doc.id, ...doc.data() }));
          events.sort((a, b) => String(b.createdAt || b.date || '').localeCompare(String(a.createdAt || a.date || '')));
          ramEventsCache = events;
          cacheProgress.loadedEvents = events.length;
          console.log(`[RAM Cache Startup] Loaded ${events.length} events catalog records.`);

          // 3. Single load of settings from Firestore
          cacheProgress.percent = 90;
          cacheProgress.stage = 'Loading system configuration...';
          console.log('[RAM Cache Startup] Loading system configuration & building search index...');

          const settingsSnap = await db.doc(SETTINGS_DOC).get();
          metrics.firestoreReads += 1;
          if (!settingsSnap.exists) {
            await db.doc(SETTINGS_DOC).set(DEFAULT_SETTINGS);
            ramSettingsCache = { ...DEFAULT_SETTINGS };
          } else {
            ramSettingsCache = { ...DEFAULT_SETTINGS, ...settingsSnap.data() };
          }

          cacheProgress.percent = 100;
          cacheProgress.status = 'ready';
          cacheProgress.stage = 'Complete';
          cacheProgress.cacheSizeMB = calculateCacheSizeMB();
          cacheProgress.initTimeMs = Date.now() - startTime;
          cacheProgress.lastRefresh = new Date().toISOString();
          cacheProgress.error = null;

          success = true;

          const summaryLog = `RAM Cache Ready | Cached ${ramGalleryCache.length} photo(s), ${descCount} face descriptor(s), ${ramEventsCache.length} event(s) | Size: ${cacheProgress.cacheSizeMB} MB | Duration: ${cacheProgress.initTimeMs} ms`;
          console.log(`[RAM Cache Startup SUCCESS] ${summaryLog}`);

          addSystemLog({
            backend: CURRENT_PLATFORM,
            eventType: 'CACHE',
            status: 'SUCCESS',
            details: summaryLog
          });

          // Sync cache status with Firestore HA status doc
          isRamCacheInitialized = true;
          await updateBackendCacheMetricsInFirestore();

        } catch (err) {
          lastErr = err;
          const errMsg = err && err.message ? err.message : String(err);
          const errStack = err && err.stack ? err.stack : '';
          console.error(`[RAM Cache Startup Warning] Firestore load attempt ${attempt}/${maxRetries} failed: ${errMsg}`);
          if (errStack) console.error(`[RAM Cache Error Stack]:\n${errStack}`);

          if (attempt < maxRetries) {
            const backoffMs = attempt * 1000;
            console.log(`[RAM Cache Retry] Waiting ${backoffMs}ms before retrying Firestore connection...`);
            await new Promise(r => setTimeout(r, backoffMs));
          }
        }
      }

      if (!success) {
        const failMsg = `Firestore load failed after ${maxRetries} attempts: ${lastErr ? lastErr.message : 'Unknown error'}. Falling back to local files.`;
        console.error(`[RAM Cache Startup FAILURE] ${failMsg}`);
        addSystemLog({
          backend: CURRENT_PLATFORM,
          eventType: 'CACHE',
          status: 'WARNING',
          details: failMsg
        });

        ramGalleryCache = readLocalGallery();
        ramEventsCache = readLocalEvents();
        ramSettingsCache = readLocalSettings();
        cacheProgress.status = 'ready';
        cacheProgress.percent = 100;
        cacheProgress.stage = 'Loaded from local fallback';
        cacheProgress.error = failMsg;
      }
    } else {
      console.log('[RAM Cache Startup] Firebase is disabled. Loading local disk JSON backup...');
      ramGalleryCache = readLocalGallery();
      ramEventsCache = readLocalEvents();
      ramSettingsCache = readLocalSettings();
      cacheProgress.status = 'ready';
      cacheProgress.percent = 100;
      cacheProgress.stage = 'Complete (Local Fallback)';
      cacheProgress.loadedPhotos = ramGalleryCache.length;
      cacheProgress.loadedEvents = ramEventsCache.length;
      cacheProgress.loadedDescriptors = countDescriptors(ramGalleryCache);
      cacheProgress.cacheSizeMB = calculateCacheSizeMB();
      cacheProgress.initTimeMs = Date.now() - startTime;
      cacheProgress.lastRefresh = new Date().toISOString();

      const localLog = `Local JSON initial load complete | Cached ${ramGalleryCache.length} photo(s) and ${ramEventsCache.length} event(s).`;
      console.log(`[RAM Cache Startup] ${localLog}`);
      addSystemLog({
        backend: CURRENT_PLATFORM,
        eventType: 'CACHE',
        status: 'SUCCESS',
        details: localLog
      });
    }

    isRamCacheInitialized = true;
    return true;
  })();

  return ramInitPromise;
}

async function refreshRamCache() {
  isRamCacheInitialized = false;
  ramInitPromise = null;
  ramGalleryCache = null;
  ramEventsCache = null;
  ramSettingsCache = null;
  return initRamCache();
}

// ---------- Local File Helpers ----------

function readLocalGallery() {
  if (galleryMemoryCache) {
    return galleryMemoryCache;
  }
  let localData = [];
  try {
    if (!fs.existsSync(galleryDbPath)) {
      fs.writeFileSync(galleryDbPath, JSON.stringify([], null, 2));
    } else {
      const data = fs.readFileSync(galleryDbPath, 'utf8');
      localData = JSON.parse(data || '[]');
    }

    let migrated = false;
    localData.forEach(photo => {
      if (photo.status === undefined) {
        photo.status = 'approved';
        migrated = true;
      }
      if (photo.isPublic === undefined) {
        photo.isPublic = true;
        migrated = true;
      }
    });

    if (migrated) {
      fs.writeFileSync(galleryDbPath, JSON.stringify(localData, null, 2));
    }
  } catch (err) {
    if (isReadOnlyFsError(err)) {
      galleryStorageReadOnly = true;
      galleryMemoryCache = [];
      localData = galleryMemoryCache;
    } else {
      console.error('Error reading gallery database:', err);
      localData = [];
    }
  }
  return localData.sort((a, b) => String(b.timestamp || b.uploadTime || '').localeCompare(String(a.timestamp || a.uploadTime || '')));
}

async function writeLocalGallery(data) {
  return new Promise((resolve, reject) => {
    const performWrite = async () => {
      isWritingGallery = true;
      try {
        await fs.promises.writeFile(galleryDbPath, JSON.stringify(data, null, 2));
        resolve(true);
      } catch (err) {
        if (isReadOnlyFsError(err)) {
          galleryStorageReadOnly = true;
          galleryMemoryCache = data;
          console.warn('Gallery storage is read-only. Using in-memory gallery cache instead:', err.message);
          resolve(true);
        } else {
          console.error('Error writing gallery database:', err);
          reject(err);
        }
      } finally {
        isWritingGallery = false;
        if (galleryWriteQueue.length > 0) {
          const nextWrite = galleryWriteQueue.shift();
          nextWrite();
        }
      }
    };

    if (!isWritingGallery) {
      performWrite();
    } else {
      galleryWriteQueue.push(performWrite);
    }
  });
}

function readLocalEvents() {
  if (eventsMemoryCache) {
    return eventsMemoryCache;
  }
  let localData = [];
  try {
    if (!fs.existsSync(eventsDbPath)) {
      fs.writeFileSync(eventsDbPath, JSON.stringify([], null, 2));
    } else {
      const data = fs.readFileSync(eventsDbPath, 'utf8');
      localData = JSON.parse(data || '[]');
    }
  } catch (err) {
    if (isReadOnlyFsError(err)) {
      eventsMemoryCache = [];
      localData = eventsMemoryCache;
    } else {
      console.error('Error reading events database:', err);
      localData = [];
    }
  }
  return localData;
}

async function writeLocalEvents(data) {
  try {
    await fs.promises.writeFile(eventsDbPath, JSON.stringify(data, null, 2));
    return true;
  } catch (err) {
    if (isReadOnlyFsError(err)) {
      eventsMemoryCache = data;
      return true;
    }
    console.error('Error writing events database:', err);
    throw err;
  }
}

function readLocalSettings() {
  if (settingsMemoryCache) {
    return { ...DEFAULT_SETTINGS, ...settingsMemoryCache };
  }
  try {
    if (!fs.existsSync(settingsPath)) {
      fs.writeFileSync(settingsPath, JSON.stringify(DEFAULT_SETTINGS, null, 2));
      return { ...DEFAULT_SETTINGS };
    }
    const data = fs.readFileSync(settingsPath, 'utf8');
    const parsed = JSON.parse(data || '{}');
    return { ...DEFAULT_SETTINGS, ...parsed };
  } catch (err) {
    if (isReadOnlyFsError(err)) {
      settingsStorageReadOnly = true;
      settingsMemoryCache = { ...DEFAULT_SETTINGS };
      console.warn('Settings storage is read-only. Using in-memory settings fallback:', err.message);
      return settingsMemoryCache;
    }
    console.error('Error reading settings:', err);
    return { ...DEFAULT_SETTINGS };
  }
}

function writeLocalSettings(data) {
  try {
    fs.writeFileSync(settingsPath, JSON.stringify(data, null, 2));
    return true;
  } catch (err) {
    if (isReadOnlyFsError(err)) {
      settingsStorageReadOnly = true;
      settingsMemoryCache = data;
      console.warn('Settings storage is read-only. Using in-memory settings cache instead:', err.message);
      return false;
    }
    console.error('Error writing settings:', err);
    return false;
  }
}

// ---------- Gallery DB (RAM Cache + Firestore Sync) ----------

async function readGalleryDb() {
  if (!isRamCacheInitialized) {
    await initRamCache();
  }
  metrics.ramCacheHits += 1;
  return [...ramGalleryCache].sort((a, b) => String(b.timestamp || b.uploadTime || '').localeCompare(String(a.timestamp || a.uploadTime || '')));
}

async function writeGalleryDb(data) {
  if (!isRamCacheInitialized) {
    await initRamCache();
  }

  if (isFirebaseEnabled()) {
    const db = getDb();
    const newIdSet = new Set(data.map(p => p.id));
    // Remove deleted documents from Firestore
    const removedPhotos = ramGalleryCache.filter(p => !newIdSet.has(p.id));
    for (const photo of removedPhotos) {
      try {
        await db.collection(PHOTOS_COLLECTION).doc(photo.id).delete();
        metrics.firestoreDeletes += 1;
      } catch (err) {
        console.error(`[RAM Cache Sync] Firestore delete failed for ${photo.id}:`, err.message);
      }
    }

    // Upsert items into Firestore
    for (const photo of data) {
      const { id, ...rest } = photo;
      try {
        await db.collection(PHOTOS_COLLECTION).doc(id).set(rest, { merge: true });
        metrics.firestoreWrites += 1;
      } catch (err) {
        console.error(`[RAM Cache Sync] Firestore set failed for ${id}:`, err.message);
      }
    }
  } else {
    await writeLocalGallery(data);
  }

  ramGalleryCache = [...data];
  return true;
}

async function addPhoto(photo) {
  if (!isRamCacheInitialized) {
    await initRamCache();
  }

  const { id, ...rest } = photo;
  if (isFirebaseEnabled()) {
    try {
      const db = getDb();
      if (db) {
        await db.collection(PHOTOS_COLLECTION).doc(id).set(rest);
        metrics.firestoreWrites += 1;
      }
    } catch (err) {
      console.error(`[Firestore Error] Failed to write photo ${id}:`, err.message);
    }
  } else {
    const localGallery = readLocalGallery();
    localGallery.push(photo);
    await writeLocalGallery(localGallery);
  }

  // Update RAM cache immediately in memory (no re-reading from Firestore)
  const idx = ramGalleryCache.findIndex(p => p.id === id);
  if (idx !== -1) {
    ramGalleryCache[idx] = { ...photo };
  } else {
    ramGalleryCache.unshift({ ...photo });
  }

  return photo;
}

async function updatePhoto(id, patch) {
  if (!isRamCacheInitialized) {
    await initRamCache();
  }

  const existing = ramGalleryCache.find(p => p.id === id);
  if (!existing) {
    return null;
  }

  if (isFirebaseEnabled()) {
    try {
      const db = getDb();
      if (db) {
        const ref = db.collection(PHOTOS_COLLECTION).doc(id);
        await ref.update(patch);
        metrics.firestoreWrites += 1;
      }
    } catch (err) {
      console.error(`[Firestore Error] Failed to update photo ${id}:`, err.message);
    }
  } else {
    const localGallery = readLocalGallery();
    const p = localGallery.find(item => item.id === id);
    if (p) {
      Object.assign(p, patch);
      await writeLocalGallery(localGallery);
    }
  }

  // Update RAM cache immediately in memory
  Object.assign(existing, patch);
  return { ...existing };
}

async function deletePhotoRecord(id) {
  if (!isRamCacheInitialized) {
    await initRamCache();
  }

  if (isFirebaseEnabled()) {
    try {
      const db = getDb();
      if (db) {
        await db.collection(PHOTOS_COLLECTION).doc(id).delete();
        metrics.firestoreDeletes += 1;
      }
    } catch (err) {
      console.error(`[Firestore Error] Failed to delete photo ${id}:`, err.message);
    }
  } else {
    const localGallery = readLocalGallery();
    const idx = localGallery.findIndex(p => p.id === id);
    if (idx !== -1) {
      localGallery.splice(idx, 1);
      await writeLocalGallery(localGallery);
    }
  }

  // Remove from RAM cache immediately
  const idx = ramGalleryCache.findIndex(p => p.id === id);
  if (idx !== -1) {
    ramGalleryCache.splice(idx, 1);
  }
  return true;
}

async function getPhotoById(id) {
  if (!isRamCacheInitialized) {
    await initRamCache();
  }
  metrics.ramCacheHits += 1;
  const photo = ramGalleryCache.find(p => p.id === id);
  return photo ? { ...photo } : null;
}

// ---------- Events DB (RAM Cache + Firestore Sync) ----------

async function readEventsDb() {
  if (!isRamCacheInitialized) {
    await initRamCache();
  }
  metrics.ramCacheHits += 1;
  return [...ramEventsCache].sort((a, b) => String(b.createdAt || b.date || '').localeCompare(String(a.createdAt || a.date || '')));
}

async function writeEventsDb(data) {
  if (!isRamCacheInitialized) {
    await initRamCache();
  }

  if (isFirebaseEnabled()) {
    const db = getDb();
    const newIdSet = new Set(data.map(e => e.id));
    const removedEvents = ramEventsCache.filter(e => !newIdSet.has(e.id));
    for (const evt of removedEvents) {
      await db.collection(EVENTS_COLLECTION).doc(evt.id).delete();
      metrics.firestoreDeletes += 1;
    }
    for (const evt of data) {
      const { id, ...rest } = evt;
      await db.collection(EVENTS_COLLECTION).doc(id).set(rest, { merge: true });
      metrics.firestoreWrites += 1;
    }
  } else {
    await writeLocalEvents(data);
  }

  ramEventsCache = [...data];
  return true;
}

async function addEvent(event) {
  if (!isRamCacheInitialized) {
    await initRamCache();
  }

  const { id, ...rest } = event;
  if (isFirebaseEnabled()) {
    const db = getDb();
    await db.collection(EVENTS_COLLECTION).doc(id).set(rest);
    metrics.firestoreWrites += 1;
  } else {
    const localEvents = readLocalEvents();
    localEvents.push(event);
    await writeLocalEvents(localEvents);
  }

  const idx = ramEventsCache.findIndex(e => e.id === id);
  if (idx !== -1) {
    ramEventsCache[idx] = { ...event };
  } else {
    ramEventsCache.push({ ...event });
  }

  return event;
}

async function updateEvent(id, patch) {
  if (!isRamCacheInitialized) {
    await initRamCache();
  }

  const existing = ramEventsCache.find(e => e.id === id);
  if (!existing) return null;

  if (isFirebaseEnabled()) {
    const db = getDb();
    const ref = db.collection(EVENTS_COLLECTION).doc(id);
    await ref.update(patch);
    metrics.firestoreWrites += 1;
  } else {
    const localEvents = readLocalEvents();
    const evt = localEvents.find(e => e.id === id);
    if (evt) {
      Object.assign(evt, patch);
      await writeLocalEvents(localEvents);
    }
  }

  Object.assign(existing, patch);
  return { ...existing };
}

async function deleteEvent(id) {
  if (!isRamCacheInitialized) {
    await initRamCache();
  }

  if (isFirebaseEnabled()) {
    const db = getDb();
    await db.collection(EVENTS_COLLECTION).doc(id).delete();
    metrics.firestoreDeletes += 1;
  } else {
    const localEvents = readLocalEvents();
    const idx = localEvents.findIndex(e => e.id === id);
    if (idx !== -1) {
      localEvents.splice(idx, 1);
      await writeLocalEvents(localEvents);
    }
  }

  const idx = ramEventsCache.findIndex(e => e.id === id);
  if (idx !== -1) {
    ramEventsCache.splice(idx, 1);
  }
  return true;
}

async function getEventById(id) {
  if (!isRamCacheInitialized) {
    await initRamCache();
  }
  metrics.ramCacheHits += 1;
  const evt = ramEventsCache.find(e => e.id === id);
  return evt ? { ...evt } : null;
}

// ---------- Settings DB (RAM Cache + Firestore Sync) ----------

async function readSettings() {
  if (!isRamCacheInitialized) {
    await initRamCache();
  }
  metrics.ramCacheHits += 1;
  return applyEnvOverrides(ramSettingsCache || DEFAULT_SETTINGS);
}

async function writeSettings(data) {
  if (!isRamCacheInitialized) {
    await initRamCache();
  }

  const payload = { ...data };
  if (payload.googleClientSecret === '********') {
    delete payload.googleClientSecret;
    if (ramSettingsCache && ramSettingsCache.googleClientSecret) {
      payload.googleClientSecret = ramSettingsCache.googleClientSecret;
    }
  }

  if (isFirebaseEnabled()) {
    try {
      const db = getDb();
      await db.doc(SETTINGS_DOC).set(payload, { merge: true });
      metrics.firestoreWrites += 1;
    } catch (err) {
      console.error('[Firebase] Failed to write settings:', err.message);
      throw new Error(`Failed to save settings to Firebase: ${err.message}`);
    }
  } else {
    writeLocalSettings(payload);
  }

  ramSettingsCache = { ...DEFAULT_SETTINGS, ...ramSettingsCache, ...payload };
  return true;
}

// ---------- Firebase Storage ----------

async function uploadToFirebaseStorage(buffer, destinationPath, mimeType) {
  const bucket = getBucket();
  if (!bucket) throw new Error('Firebase Storage is not configured');

  const file = bucket.file(destinationPath);
  await file.save(buffer, {
    metadata: {
      contentType: mimeType,
      cacheControl: 'public, max-age=31536000'
    },
    resumable: false
  });

  try {
    await file.makePublic();
  } catch (err) {
    console.warn('[Firebase Storage] makePublic failed (check IAM). Falling back to signed URL.', err.message);
  }

  const publicUrl = `https://storage.googleapis.com/${bucket.name}/${encodeURI(destinationPath)}`;
  return { path: destinationPath, url: publicUrl };
}

async function deleteFromFirebaseStorage(storagePath) {
  const bucket = getBucket();
  if (!bucket || !storagePath) return;
  try {
    await bucket.file(storagePath).delete({ ignoreNotFound: true });
  } catch (err) {
    console.error(`[Firebase Storage] Failed to delete ${storagePath}:`, err.message);
  }
}

async function getFirebaseFileStream(storagePath) {
  const bucket = getBucket();
  if (!bucket) throw new Error('Firebase Storage is not configured');
  const file = bucket.file(storagePath);
  const [exists] = await file.exists();
  if (!exists) throw new Error('File not found');
  const [meta] = await file.getMetadata();
  return {
    stream: file.createReadStream(),
    contentType: meta.contentType || 'application/octet-stream'
  };
}

async function reconnectFirestore() {
  isRamCacheInitialized = false;
  ramInitPromise = null;
  const { initFirebase } = require('./firebase');
  initFirebase();
  return initRamCache();
}

async function rebuildSearchIndex() {
  return refreshRamCache();
}

module.exports = {
  isFirebaseEnabled,
  initRamCache,
  refreshRamCache,
  reconnectFirestore,
  rebuildSearchIndex,
  getCacheStatus,
  getMetrics,
  driveMetrics,
  isDuplicateRequest,
  readGalleryDb,
  writeGalleryDb,
  addPhoto,
  updatePhoto,
  deletePhotoRecord,
  getPhotoById,
  readEventsDb,
  writeEventsDb,
  addEvent,
  updateEvent,
  deleteEvent,
  getEventById,
  readSettings,
  writeSettings,
  invalidateSettingsCache,
  sanitizeSettingsForClient,
  uploadToFirebaseStorage,
  deleteFromFirebaseStorage,
  getFirebaseFileStream,
  getHAStatus,
  setActiveBackend,
  toggleAutoFailover,
  updateBackendCacheMetricsInFirestore,
  DEFAULT_SETTINGS
};
