...
async function initRamCache() {
  ...
  addSystemLog({
    ...
    eventType: 'CACHE',
    status: 'INFO',
    details: 'Starting metadata RAM cache initialization...'
  });
  ...
  ramInitPromise = (async () => {
    if (isFirebaseEnabled()) {
      try {
        ...
        cacheProgress.loadedDescriptors = countDescriptors(photos);
        ...
        addSystemLog({
          ...
          eventType: 'CACHE',
          status: 'SUCCESS',
          details: `Firestore initial load successful. Cached ${ramGalleryCache.length} photo(s), ${cacheProgress.loadedDescriptors} face descriptor(s), ${ramEventsCache.length} event(s).`
        });
      } catch (err) {
        addSystemLog({
          backend: CURRENT_PLATFORM,
          eventType: 'CACHE',
          status: 'WARNING',
          details: `Firestore load failed, falling back to local files: ${err.message}`
        });
      ...
      addSystemLog({
        ...
        eventType: 'CACHE',
        status: 'SUCCESS',
        details: `Local JSON initial load successful. Cached ${ramGalleryCache.length} photo(s) and ${ramEventsCache.length} event(s).`
      });
    }
